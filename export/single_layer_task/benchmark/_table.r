...
save.table = function(dat.last.rate, label, caption, file.out, show.operation=TRUE, highlight.best=TRUE, longtable=FALSE) {
  ...
  align = c('c', 'r', 'r', 'l', 'l', 'l', 'l')
  header.1 = c("Op", "Model", "Range", "Success", "Solved at", "Sparsity error")
  header.2 = c("", "", "", "Rate", "Mean", "Mean")
  ...
  dat.table = dat.last.rate %>%
    ...
    select(operation, model, parameter, success.rate, converged.at.mean, sparse.error) %>%
    arrange(operation, model, parameter)
  ...
  dat.table %>%
    ...
    collapse_rows(columns = ifelse(show.operation, c(1), c(1,2)), latex_hline = "major") %>%
    # adding collapse here will break the op collapse
    #collapse_rows(columns = c(2), latex_hline = "major") %>%
    write(file.out)
}
